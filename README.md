...
None of this needs WordPress — it's about a few one-time steps, not the
platform:

1. **Submit to Google Search Console** (search.google.com/search-console,
   free). Add `cjxrobot.github.io` as a property, verify via the HTML
   meta tag method, and submit `https://cjxrobot.github.io/sitemap.xml`
   (auto-generated by the `jekyll-sitemap` plugin now included here).
2. **Link to it from places people already find you** — your GitHub
   profile README, LinkedIn, any RoboCup/FH Robotronics socials. Search
   engines weight inbound links heavily; this matters more than most
   on-page tweaks.
3. **Keep posting to the log.** Fresh, dated content is one of the
   stronger signals for re-crawling and ranking a personal site.
4. Page titles/descriptions are already handled by `jekyll-seo-tag`
   (already in this repo) — it pulls from each page's title and the
   site description automatically.
